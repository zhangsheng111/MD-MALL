...
var vm = new Vue({
    ...
    // QQ登陆成功跳转到保存页面的同时, 也就是该页面启动时,向后端发送请求(code,state)
   mounted: function(){
        // 从路径中获取qq重定向返回的code(QQ登陆成功,QQ服务器即返回该用户的code)
        var code = this.get_query_string('code');  // 获取当前网页中的code值
        axios.get(this.host + '/oauth/qq/user/?code=' + code, {
            ...
            .then(response => {
                if (response.data.user_id){
                    ...
                    this.generate_image_code();  //获取图片验证码
